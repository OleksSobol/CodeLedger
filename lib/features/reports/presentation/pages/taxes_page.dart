...
class _TaxesPageState extends ConsumerState<TaxesPage>
  ...
  Widget _buildQuarterlyCards(
    ...
    // Only deductible receipts reduce net income
    final totalExpenses = _receipts
        .where((r) => r.isTaxDeductible)
        .fold(0.0, (sum, r) => sum + r.amount);
    final totalMonthlyDeductible =
        ref.watch(totalMonthlyDeductibleProvider);
    final now = DateTime.now();

    // IRS estimated quarterly payment schedule
    final quarters =
        <({String label, String dates, DateTime start, DateTime end, DateTime due, int months})>[
    ...
    return Column(
      ...
      children: [
        ...
        ...quarters.map((q) {
          ...
          // Prorate receipt deductions evenly + recurring monthly deductions
          final receiptAlloc = totalExpenses / 4;
          final recurringAlloc = totalMonthlyDeductible * q.months;
